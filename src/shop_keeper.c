...
Entity* shop_keeper_new_entity(GFC_Vector2D position)
{
	...
	if (!self)
	{
		slog("failed to spawn a shop_keeper entity");
		return NULL;
	}
	gfc_vector2d_copy(self->position, position);
	self->think = shop_keeper_think;
	self->update = shop_keeper_update;
...
void shop_keeper_move(Entity* self)
{
	...
	if ((shop_keeper_see_player(self)) && (self->statusEffects->freezeEffect == 0))
	{
		self->velocity.x = -1;
...
{
	if (!self) return;
	shop_keeper_damage(self);
	shop_keeper_move(self);
	self->frame += 0.05f;
	//slog("Frame is: %f", self->frame);
	if (self->frame >= 4) self->frame = 0;
	shop_keeper_attack(self);
	shop_keeper_status(self);
...
void shop_keeper_attack(Entity* self)
{
	if (!self)return;
	GFC_Rect playerBounds = get_player_bounds();
	if (shop_keeper_see_player(self))
	{

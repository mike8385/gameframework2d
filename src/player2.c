...
static Player2System player2_system = { 0 }; /**<Initalize a LOCAL global entity manager*/
...
Entity* player2_new_entity(GFC_Vector2D position)
{
	...
	if (!self)
	{
		slog("failed to spawn a player2 entity");
		return NULL;
	}
	...
	//self->pinventory = gfc_allocate_array(sizeof(Player2Inventory), 1);
	//if (self->pinventory)
	//{
	...
	player2_system.hasPet = 0;
	self->isFollowing = 0;
	self->hasPet = PT_None;
	self->shadow  = gf2d_sprite_load_all("images/shadow.png", 128, 64, 1, 0);
	strcpy(self->name, "Player2");

	player2_system.player2Data = self;
	player2_system.player2Stats = self->data;
...
void player2_move(Entity *self)
{
	...
	if (keys[SDL_SCANCODE_RIGHT])		//gfc_input_command_down("right")
	{
		data = get_player2_stats();
...
void player2_attack(Entity* self)
{
	...
	if (keys[SDL_SCANCODE_INSERT] && (self->magicCooldown == 0))
	{
	...
	if (keys[SDL_SCANCODE_HOME] && (self->magicCooldown == 0) && (stats->magic >= 2))
	{
...
{
	return player2_system.player2Data->bounds;
}

GFC_Vector2D get_player2_position()
{
	return player2_system.player2Data->position;
}

Stats* get_player2_stats()
{
	return player2_system.player2Data->data;
}

float get_player2_health()
{
	return player2_system.player2Data->health;
}

Entity* player2_get_player2()
{
	return player2_system.player2Data;
}

Uint8 get_player2_pet()
{
	return player2_system.hasPet;
}
